...
- Query Supabase: `supabase.from('tasks').select('*, companies(name)')` -- do NOT use `!inner` (follow the same convention as companies.ts which uses `select('*, tasks(id, is_completed)')` without `!inner`)
- Chain `.lte('deadline', today)` to fetch tasks with deadline <= today (captures all overdue + today tasks)
- Type the cast to handle nullable companies relation: `(row as unknown as Task & { companies: { name: string } | null })`
- Map result to TaskWithCompany with null-safe company name: `company_name: castRow.companies?.name ?? 'Onbekend'`
- This defensive null handling prevents runtime errors even though company_id is non-nullable in the schema
...
- Include task if: `overdue.effectiveDeadline === today`
- This single condition correctly handles ALL cases:
  - Overdue incomplete tasks: computeOverdue rolls effectiveDeadline to today, so effectiveDeadline === today is true
  - Incomplete tasks due today: effectiveDeadline = deadline = today, so effectiveDeadline === today is true
  - Completed tasks due today: computeOverdue returns effectiveDeadline = deadline (no rollover for completed), so effectiveDeadline === today is true
  - Completed tasks due in the past: effectiveDeadline = past deadline, so effectiveDeadline === today is false (correctly excluded)
  - Future tasks: effectiveDeadline = future deadline, so effectiveDeadline === today is false (correctly excluded)
- Do NOT add a redundant OR condition like `task.is_completed && task.deadline === today` -- that case is already covered by the effectiveDeadline check above
...
- filterTodayTasks includes overdue tasks (effective deadline = today) AND completed tasks with deadline = today, using the single condition `overdue.effectiveDeadline === today`
- getTodayTasksWithCompany fetches tasks joined with company names from Supabase, with null-safe company name handling
