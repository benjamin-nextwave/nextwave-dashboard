  ...
  <action>
    1. Create `src/components/navigation.tsx` as a 'use client' component:
       ...
       - Render a nav element with:
         ...
         e) Active detection: use exact match for all routes: `pathname === item.href`. This works correctly for all three routes ('/', '/gantt', '/bedrijven') because none have sub-routes. Do NOT use startsWith -- it would cause '/' to match every page.
